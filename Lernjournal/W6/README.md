...
Man kann `tfstate`-Dateien in einem Azure Storage Account speichern. Allerdings kann man nicht einfach einen Storage Account per Terraform definieren und erstellen und diesen anschliessend im selben Terraform-Workflow als Remote Backend verwenden. Das bedeutet, ich muss ein separates Deployment durchführen, um diesen Storage Account bereitzustellen.
...
Ich habe eingetragen, dass ich aktuell "at risk" bin, aber keine Hilfe benötige, da ich genau weiss, was zu tun ist, ich muss es nur noch zum Laufen bringen.
